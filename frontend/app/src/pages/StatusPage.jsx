...
const StatusPage = () => {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                ...
                // prefer RUNNING, otherwise prefer the most-recent session that was active
                // (PAUSED, FINISHED or ERROR) so the Status page still shows the last active
                // session's counts after the session is stopped.
                let active = null
                if (Array.isArray(sessions) && sessions.length > 0) {
                    active = sessions.find(s => s.status === 'RUNNING') ||
                        sessions.find(s => ['PAUSED', 'FINISHED', 'ERROR'].includes(s.status)) ||
                        null
                }
                ...
                const [detail, inv] = await Promise.all([
                    sessionAPI.getSessionDetail(active.id),
                    inventoryAPI.getInventory()
                ])

                setSession(detail.session)

                // session detail returns items: [{ ball_type_id, name, count }, ...]
                setItems(detail.items || [])

                // store inventory for ball type names (we do NOT show expected counts here)
                setInventory(Array.isArray(inv) ? inv : [])
            } catch (err) {
                console.error('Failed to load session for status page', err)
            }
        }
        load()

        // refresh every 3s while page visible
        const interval = setInterval(load, 3000)
        return () => clearInterval(interval)
    }, [])

    // Listen for optimistic status changes from other components (BottomBar)
    useEffect(() => {
        let mounted = true
        const onSessionStatusChanged = async (e) => {
            const detail = (e && e.detail) || {}
            const { status, sessionId } = detail

            // If we already have a session loaded, update its status immediately
            if (!mounted) return
            if (session) {
                // Map `null` -> FINISHED (keep last-session counts but show Idle)
                if (status === null) {
            ...
            // If no session is present but an id was provided, fetch it to display immediately
            if (sessionId) {
                try {
                    ...
                    // ignore — periodic loader will reconcile
                    console.warn('failed to fetch session after optimistic status change', err)
                }
            }
        }

        window.addEventListener('sessionStatusChanged', onSessionStatusChanged)
        return () => {
            mounted = false
            window.removeEventListener('sessionStatusChanged', onSessionStatusChanged)
        }
    }, [session])

    // Also listen for direct clicks on the SessionControls buttons so the Status text updates instantly
    useEffect(() => {
        let mounted = true
        const onDocClick = (ev) => {
            if (!mounted) return
            ...
            if (target.closest && target.closest('.pauseBut')) {
    ...
    const itemsByType = Object.fromEntries((items || []).map(i => [i.ball_type_id, i]))
    const totalCount = Object.values(itemsByType).reduce((s, it) => s + (it.count || 0), 0)
                    ...
                    <div className="session-ball-box">
                        ...
                        <div className="session-ball">
                            ...
                            <div className="ball-values">
                                <p>{(itemsByType[1] || {}).count || 0}</p>
                            </div>
                        </div>

                        <div className="session-ball">
                            <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="40px" fill={footballColor}>
                            ...
                            <p>Soccer Balls</p>
                            <div className="ball-values">
                                <p>{(itemsByType[2] || {}).count || 0}</p>
                            </div>
                        </div>

                        <div className="session-ball">
                            <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="40px" fill={volleyballColor}>
